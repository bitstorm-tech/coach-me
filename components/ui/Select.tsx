    ...
    <div ref={ref}>
      ...
      <div className="flex items-center relative">
        ...
        <svg
          ...
          className="h-6 w-6 text-primary cursor-pointer absolute right-2"
          onClick={toggleOpen}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
      ...
      {isOpen && (
        <div className="absolute bg-dark mt-1 border rounded border-darkest">
          {filteredOptions.map((option, i) => (
            <div
              key={i}
              onClick={() => select(option)}
              className="my-1 px-4 cursor-pointer text-primary"
            >
              {option}
            </div>
          ))}
